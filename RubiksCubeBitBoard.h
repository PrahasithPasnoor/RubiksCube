...
private:
    ...
    void rotateFace(int ind) {  // clockwise roatation
        uint64_t side = bitboard[ind];
        side = side >> (8 * 6); // left most 16 bits
        bitboard[ind] = (bitboard[ind] << 16) | (side); // leftshift removes the side part, added at front
    }
...
public:
    ...
    RubiksCubeBitBoard() {  // constructer creates cubes initial solved configuration
        // side implies a face
        for (int side = 0; side < 6; side++) {
            ...
            for (int faceIdx = 0; faceIdx < 8; faceIdx++) {
                bitboard[side] |= clr << (8 * faceIdx); // indexing of colors on face right to left 
            }
    ...
    COLOR getColor(FACE face, unsigned row, unsigned col) const override {
        int idx = arr[row][col];    // index in face is given by this array
        if (idx == 8) return (COLOR)((int) face);   // center piece is same as face

        uint64_t side = bitboard[(int) face]; // like UP is 0, LEFT is 1
        uint64_t color = (side >> (8 * idx)) & one_8; // get color at index idx

        int bit_pos = 0;
        while (color != 1) {    // getting to know color from the 8 bits
            color = color >> 1;
            bit_pos++;
        }
        return (COLOR)(bit_pos);    // ex: right most bit 1 is white
    }
    ...
    RubiksCube &u() override {
        this->rotateFace(0);    // U face rotation
        // 0,1,2 clr rotation on faces 1,2,3,4
